...
def _staticcheck_call(c: Call) -> None:
    if not re.match(r"^[a-z][a-zA-Z0-9_]*$", c.name):
        raise StaticException("Function call to '%s' does not match regex "
...
def _typecheck_call(c: Call, ctx: TypeContext) -> None:
    ...
    if len(c.args) != len(t_in):
        raise TypeException("Function call to '%s' expects %s arguments, not %s"
                %(c.name, len(t_in), len(c.args)))
    for i, arg in enumerate(c.args):
        typecheck(arg, ctx)
        if arg.get_type() != t_in[i]:
            raise TypeException("Argument %s of call to '%s' should be of type"
...
def _codegen_expression_call(c: Call, builder: ir.IRBuilder, ctx: CodegenContext) \
        -> ir.Value:
    return _codegen(c, builder, ctx)

@codegen_statement.register(Call)
def _codegen_statement_call(c: Call, builder: ir.IRBuilder, ctx: CodegenContext) \
        -> None:
