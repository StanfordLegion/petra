...
def _typecheck_comparison(c: Comparison, ctx: TypeContext) -> None:
    ...
    if (t_left, t_right) in ((Int8_t, Int8_t), (Int32_t, Int32_t)):
        ...
        raise TypeException("Incompatible types for arithmetic comparison: %s and %s"
...
def _codegen_expression_less_than(c: Lt, builder: ir.IRBuilder, ctx: \
...
def _codegen_expression_less_than_or_equal(c: Lte, builder: ir.IRBuilder, ctx: \
        CodegenContext) -> ir.Value:
    left = codegen_expression(c.left, builder, ctx)
    right = codegen_expression(c.right, builder, ctx)
    return builder.icmp_signed("<=", left, right)
...
def _codegen_expression_greater_than(c: Gt, builder: ir.IRBuilder, ctx: \
        CodegenContext) -> ir.Value:
    left = codegen_expression(c.left, builder, ctx)
    right = codegen_expression(c.right, builder, ctx)
    return builder.icmp_signed(">", left, right)
...
def _codegen_expression_greater_than_or_equal(c: Gte, builder: ir.IRBuilder, ctx: \
        CodegenContext) -> ir.Value:
    left = codegen_expression(c.left, builder, ctx)
    right = codegen_expression(c.right, builder, ctx)
    return builder.icmp_signed(">=", left, right)
...
def _typecheck_equality_comparison(c: EqualityComparison, ctx: TypeContext) -> None:
    ...
    if (t_left, t_right) in ((Bool_t, Bool_t), (Int8_t, Int8_t), (Int32_t, Int32_t)):
        ...
        raise TypeException("Incompatible types for equality comparison: %s and %s"
                %(str(t_left), str(t_right)))
...
def _codegen_expression_equal(c: Eq, builder: ir.IRBuilder, ctx: \
        CodegenContext) -> ir.Value:
    left = codegen_expression(c.left, builder, ctx)
    right = codegen_expression(c.right, builder, ctx)
    return builder.icmp_signed("==", left, right)
...
def _codegen_expression_not_equal(c: Neq, builder: ir.IRBuilder, ctx: \
        CodegenContext) -> ir.Value:
    left = codegen_expression(c.left, builder, ctx)
    right = codegen_expression(c.right, builder, ctx)
    return builder.icmp_signed("!=", left, right)
...
def _typecheck_boolean_binop(b: BooleanBinop, ctx: TypeContext) -> None:
    ...
    if (t_left, t_right) == (Bool_t, Bool_t):
        ...
        raise TypeException("Incompatible types for boolean binary operation: %s and %s"
                %(str(t_left), str(t_right)))
...
def _codegen_expression_and(b: And, builder: ir.IRBuilder, ctx: \
        CodegenContext) -> ir.Value:
...
def _codegen_expression_or(b: Or, builder: ir.IRBuilder, ctx: \
        CodegenContext) -> ir.Value:
    left = codegen_expression(b.left, builder, ctx)
    temp = builder.alloca(convert_type(Bool_t))
    builder.store(left, temp)
...
def _codegen_expression_not(b: Not, builder: ir.IRBuilder, ctx: \
        CodegenContext) -> ir.Value:
